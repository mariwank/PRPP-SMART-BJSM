##### This code runs a BJSM model when the multiplicative assumptions of BJSM are violated
##### It requires a separate file because we cannot evaluate certains elements of the model 
##### (i.e, linkage parameters) in this sub-scenario. 
...
true_piDTR_mat <- matrix(c(pi_A, pi_B, pi_AC, pi_AD, pi_BC, pi_BD, expected_pref))
rownames(true_piDTR_mat) <- c("PiA", "PiB", "PiAC", "PiAD", "PiBC", "PiBD", "AAC00", "AAD00", "BBC00", "BBD00", "AAC01", "AAD01", "BBC01", "BBD01", "AAC10", "AAD10", "BBC10", "BBD10", "AAC11", "AAD11", "BBC11", "BBD11")
...
for (s in scenario_vec) {
  ...
  # BJSM Set-Up
    ...
    for (i in 1:n.sim) {
      ...
      alpha_hat_bjsm <- rbind(alpha_hat_bjsm, store$mean$alphaP) # 2 columsn n.sim rows "AlphaP1" (1st stage preference), "AlphaP2 (2nd stage preference)
      
      # alpha estimates posterior variances
      alpha_hat_bjsm_var <- rbind(alpha_hat_bjsm_var, store$sd$alphaP^2) # 2 columsn n.sim rows "AlphaP1" (1st stage preference), "AlphaP2 (2nd stage preference)
      
      # Coverage 
      quantile_mat <- mcmc.out$quantiles[c(1:6,8:29),c(1,5)] 
      # reorder quantile mat to be in the same order as true_piDTR_mat
      rowname_order <- c("pi[1]", "pi[2]", "pi[3]", "pi[4]", "pi[5]", "pi[6]", "DTR[1]", "DTR[2]", "DTR[3]", "DTR[4]", "DTR[5]", "DTR[6]", "DTR[7]", "DTR[8]", "DTR[9]", "DTR[10]", "DTR[11]", "DTR[12]", "DTR[13]", "DTR[14]", "DTR[15]", "DTR[16]")
    ...
    ## coverage 
    params <- c("PiA", "PiB", "PiAC", "PiAD", "PiBC", "PiBD", "AAC00", "AAD00", "BBC00", "BBD00", "AAC01", "AAD01", "BBC01", "BBD01", "AAC10", "AAD10", "BBC10", "BBD10", "AAC11", "AAD11", "BBC11", "BBD11")
